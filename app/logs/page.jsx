...
export default function Logs() {
  ...
  const filterLogsByDate = (log) => {
    const now = new Date();
    const logDate = log.timestamp instanceof Date ? log.timestamp : new Date(log.timestamp?.seconds * 1000);
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        if (sortConfig.key === 'timestamp') {
          aValue = a.timestamp instanceof Date ? a.timestamp.getTime() : new Date(a.timestamp?.seconds * 1000).getTime();
          bValue = b.timestamp instanceof Date ? b.timestamp.getTime() : new Date(b.timestamp?.seconds * 1000).getTime();
  ...
  const getUserActivityStats = (userId) => {
    ...
    return {
      ...
      lastActivity: userLogs[0]?.timestamp ? 
        (userLogs[0].timestamp instanceof Date ? 
          userLogs[0].timestamp.toLocaleString() : 
          new Date(userLogs[0].timestamp?.seconds * 1000).toLocaleString()) : 
        'None'
    };
  };
